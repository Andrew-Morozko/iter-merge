...
impl<T, A: Array<Item = T>> Storage for StackVec<A> {
    ...
    #[inline(always)]
    fn reserve_for<I: Iterator>(&mut self, _iter: &I) {
...
mod tests {
    ...
    #[test]
    fn test_new() {
        let stack_vec: StackVec<[i32; 10]> = Storage::new(); // capacity parameter is ignored
    ...
    fn test_reserve_for() {
        let iter = 0..5;
        let mut stack_vec: StackVec<[i32; 10]> = Storage::new();
    ...
    #[test]
    fn test_push() {
        let mut stack_vec: StackVec<[i32; 10]> = StackVec::new();
        stack_vec.extend([1, 2, 3]);
        Storage::push(&mut stack_vec, 42);
    ...
    fn test_remove() {
        ...
        let removed = Storage::remove(&mut stack_vec, 2);
        assert_eq!(removed, 3);
    ...
    fn test_swap_remove() {
        let mut stack_vec: StackVec<[i32; 10]> = StackVec::new();
        stack_vec.extend([1, 2, 3, 4, 5]);
        let removed = Storage::swap_remove(&mut stack_vec, 1);
        assert_eq!(removed, 2);
    ...
    fn test_len() {
        let mut stack_vec: StackVec<[i32; 10]> = StackVec::new();
        stack_vec.extend([1, 2, 3, 4, 5]);
        assert_eq!(Storage::len(&stack_vec), 5);
    }
    ...
    fn test_get() {
        ...
        assert_eq!(*Storage::get(&stack_vec, 0), 42);
        assert_eq!(*Storage::get(&stack_vec, 1), 100);
        assert_eq!(*Storage::get(&stack_vec, 2), 200);
    }
    ...
    fn test_get_mut() {
        let mut stack_vec: StackVec<[i32; 10]> = StackVec::new();
        stack_vec.extend([42, 100, 200]);
        *Storage::get_mut(&mut stack_vec, 0) = 300;
        *Storage::get_mut(&mut stack_vec, 2) = 400;
