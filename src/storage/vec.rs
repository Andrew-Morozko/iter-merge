...
mod tests {
    ...
    #[test]
    fn test_new() {
        let vec: Vec<i32> = Storage::new(SealedToken);
    ...
    fn test_reserve_for() {
        let iter = 0..5;
        let mut stack_vec: Vec<i32> = Storage::new(SealedToken);
    ...
    fn test_push() {
        ...
        Storage::push(&mut vec, 42, SealedToken);
    ...
    fn test_remove() {
        ...
        let removed = Storage::remove(&mut vec, 2, SealedToken);
        assert_eq!(removed, 3);
    ...
    fn test_swap_remove() {
        let mut vec = vec![1, 2, 3, 4, 5];
        let removed = Storage::swap_remove(&mut vec, 1, SealedToken);
        assert_eq!(removed, 2);
    ...
    fn test_len() {
        ...
        assert_eq!(Storage::len(&vec, SealedToken), 5);
    }

    #[test]
    fn test_get() {
        let vec = vec![42, 100, 200];
        assert_eq!(*Storage::get(&vec, 0, SealedToken), 42);
        assert_eq!(*Storage::get(&vec, 1, SealedToken), 100);
        assert_eq!(*Storage::get(&vec, 2, SealedToken), 200);
    }

    #[test]
    fn test_get_mut() {
        let mut vec = vec![42, 100, 200];
        *Storage::get_mut(&mut vec, 0, SealedToken) = 300;
        *Storage::get_mut(&mut vec, 2, SealedToken) = 400;
