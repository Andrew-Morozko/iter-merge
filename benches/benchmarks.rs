...
fn bench_itertools(c: &mut Criterion) {
    ...
    for n_iters in iter_counts.iter().copied() {
        ...
        group.bench_function(BenchmarkId::new("IterMerge", n_iters), |b| {
            b.iter(|| {
                Merged::new(
                    (0..n_iters).map(|iter_n| {
                        vec.iter().skip(iter_n * it_len).take(it_len).copied()
                    })
                ).arbitrary_tie_breaking().build().into_vec()
            });
        });
...
fn bench_configs(c: &mut Criterion) {
    ...
    let vec = StdRng::seed_from_u64(0).random_iter().take(n_els).collect::<Vec<u64>>();

    let mut group = c.benchmark_group(format!("Configs ({n_els} items; {N_ITERS} iters{})", if cfg!(feature = "forbid_unsafe") {"; forbid_unsafe"} else {""}));
